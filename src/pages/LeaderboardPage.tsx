...
const LeaderboardPage: React.FC = () => {
  ...
  const [metric, setMetric] = useState<
    'total_points' | 'total_hunts_completed'
  >('total_points');
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-yellow-50 to-orange-50 py-10 font-sans">
      <div className="container mx-auto px-4 md:px-6 lg:px-8">
        <header className="mb-10 text-center">
          ...
          <div className="mt-6">
            <Button
              variant={metric === 'total_points' ? 'primary' : 'outline'}
            ...
            <Button
              variant={
                metric === 'total_hunts_completed' ? 'primary' : 'outline'
              }
              onClick={() => setMetric('total_hunts_completed')}
            >
              Most Hunts Solved
            </Button>
          </div>
        ...
        {loading && (
          <div className="flex justify-center py-10">
            <Loader message="Fetching legends..." size="lg" />
          </div>
        )}
        {error && (
          ...
          <Card className="shadow-xl max-w-2xl mx-auto">
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-slate-700">
                Top Players by{' '}
                {metric === 'total_points' ? 'Points' : 'Hunts Completed'}
              </CardTitle>
            </CardHeader>
            <CardContent>
              {leaderboard.length === 0 ? (
                ...
                <ul className="divide-y divide-slate-200">
                  {leaderboard.map((entry, index) => (
                    <li
                      key={entry.user_id}
                      className="py-3 px-1 flex items-center justify-between hover:bg-slate-50 transition-colors"
                    >
                      <div className="flex items-center">
                        <span className="text-lg font-semibold text-slate-500 w-8 text-center">
                          {entry.rank || index + 1}.
                        </span>
