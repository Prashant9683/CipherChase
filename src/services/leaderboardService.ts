...
export const fetchGlobalLeaderboard = async (metric: 'total_points' | 'total_hunts_completed' = 'total_points', limit: number = 10): Promise<LeaderboardEntry[]> => {
    ...
    return (data || []).map((entry, index) => ({
        ...
        score: metric === 'total_points' ? entry.total_points : entry.total_hunts_completed,
        rank: index + 1,
    }));
};
