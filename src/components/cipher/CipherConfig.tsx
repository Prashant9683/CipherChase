...
const CipherConfig: React.FC<CipherConfigProps> = ({
  cipherType,
  config,
  onConfigChange,
}) => {
        ...
        return (
          <div>
            ...
            <Input
              ...
              value={config.shift || ''}
              onChange={(e) =>
                onConfigChange('shift', parseInt(e.target.value, 10))
              }
              placeholder="e.g., 3"
              className="mt-1 w-full"
            />
            <p className="text-xs text-slate-500 mt-1">
              How many positions to shift each letter (1-25).
            </p>
          </div>
        );
      case 'substitution':
        return (
          <div>
            ...
            />
            <p className="text-xs text-slate-500 mt-1">
              A 26-letter key representing the new alphabet (A → 1st letter, B →
              2nd, etc.).
            </p>
          </div>
        );
        ...
        return (
          <div>
            ...
            <Input
              ...
              onChange={(e) =>
                onConfigChange('columns', parseInt(e.target.value, 10))
              }
              placeholder="e.g., 5"
              className="mt-1 w-full"
            />
            <p className="text-xs text-slate-500 mt-1">
              Number of columns for the transposition grid (2-10).
            </p>
          </div>
        );
      case 'riddle':
      case 'anagram': // Anagrams might not need specific config but might need expected answer
        return (
          <div>
            ...
            />
            <p className="text-xs text-slate-500 mt-1">
              The exact answer that will be considered correct.
            </p>
          </div>
        );
      ...
      case 'morse':
        return (
          <p className="text-sm text-slate-600 p-3 bg-blue-50 border border-blue-200 rounded-md">
            No additional configuration needed for this cipher type beyond the
            text to encrypt/decrypt. If this is part of a puzzle, ensure you
      ...
      default:
        return (
          <p className="text-sm text-slate-500">
            Select a cipher to see its configuration options.
          </p>
        );
    }
  };
    ...
    <div className="p-6 bg-white rounded-xl shadow-lg border border-slate-200">
      <h3 className="text-lg font-semibold text-slate-700 mb-4 flex items-center font-serif">
        ...
        <span className="text-blue-600 ml-1">
          {cipherInfo[cipherType]?.name || 'Cipher'}
        </span>
      </h3>
