...
export const StoryNodeListEditor: React.FC<StoryNodeListEditorProps> = ({
  nodes,
  setNodes,
  puzzles,
  setPuzzles,
}) => {
  const [editingNodeClientId, setEditingNodeClientId] = useState<string | null>(
    null
  );
  ...
  const addNode = (type: StoryNode['node_type']) => {
    ...
    if (type === 'narrative_block' || type === 'story_update')
      content = { text: '' };
    else if (type === 'dialogue_message')
      content = { sender_name: '', message_text: '' };
    else if (type === 'visual_cue')
      content = { image_url: '' }; // Or relevant defaults
    else if (type === 'player_choice') content = { prompt: '' };
    else if (type === 'puzzle_interaction')
      content = { puzzle_id: '', success_node_id: '' };
    else if (type === 'action_trigger')
      content = {
        action_label: '',
        success_node_id: '',
      }; // Default for action_trigger
  ...
  const updateNodeContent = (
    ...
    setNodes((prev) =>
      prev.map((node) =>
        node.client_id === clientId
          ? { ...node, content: { ...node.content, ...contentUpdates } }
          : node
      )
    );
  };

  const updateNodeChoices = (
    clientId: string,
  ...
  const deleteNode = (clientId: string) => {
    ...
    if (nodeToDelete && nodeToDelete.is_starting_node && nodes.length > 1) {
      alert(
        'Cannot delete the starting node if other nodes exist. Designate another starting node first or delete all other nodes.'
      );
      return;
    }
    const isLinked = nodes.some(
      (n) =>
    ...
    if (isLinked) {
      if (
        !window.confirm(
          'Warning: Other nodes or choices link to this node. Deleting it may break story flow. Are you sure?'
        )
      ) {
        return;
      }
  ...
  const editingNode = nodes.find(
    (node) => node.client_id === editingNodeClientId
  );
    ...
    <Card className="bg-white shadow-lg">
      <CardHeader className="flex flex-wrap justify-between items-center gap-2">
        ...
        {!startingNodeExists && nodes.length > 0 && (
          <p className="w-full text-xs text-red-500 mt-2 flex items-center">
            <AlertTriangle size={14} className="mr-1" /> Warning: No starting
            node designated!
          </p>
        )}
      </CardHeader>
      <CardContent>
        {nodes.length === 0 ? (
          <p className="text-slate-500 text-center py-6">
            No story nodes yet. Add one to begin!
          </p>
        ) : (
              ...
              .map((node, index) => (
                <li
                  ...
                  className={`p-3 border rounded-md ${
                    editingNodeClientId === node.client_id
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <GripVertical
                        size={18}
                        className="text-slate-400 mr-1.5 cursor-move"
                      />
                      <span className="font-medium text-sm text-slate-700">
                        ...
                        {node.is_starting_node && (
                          <span className="ml-2 text-xs text-green-700 bg-green-100 px-1.5 py-0.5 rounded-full">
                            START
                          </span>
                        )}
                      </span>
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          setEditingNodeClientId(
                            node.client_id === editingNodeClientId
                              ? null
                              : node.client_id
                          )
                        }
                      ...
                      <Button
                        size="icon"
                        variant="ghost"
                        onClick={() => deleteNode(node.client_id)}
