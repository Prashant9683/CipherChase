...
export type ClientStoryNode = Omit<
  StoryNode,
  'id' | 'hunt_id' | 'created_at' | 'story_node_choices'
> & {
  client_id: string;
  choices?: Array<
    Omit<
      HuntChoice,
      'id' | 'story_node_id' | 'created_at' | 'display_order'
    > & {
      client_id: string;
...
export type ClientPuzzle = Omit<
  Puzzle,
  'id' | 'created_at' | 'creator_id' | 'post_solve_story_node_id'
> & {
  client_id: string;
...
const initialHuntMetadata: Partial<
  Omit<
    TreasureHunt,
    | 'id'
    | 'creator'
...
const HuntCreator: React.FC = () => {
  ...
  const handleSaveHunt = async () => {
    ...
    if (!huntMetadata.title?.trim()) {
      setError('Hunt title is required.');
      return;
    }
    if (storyNodes.length === 0) {
      setError('At least one story node is required.');
      return;
    }
    const startingNodes = storyNodes.filter((node) => node.is_starting_node);
    if (startingNodes.length !== 1) {
      setError('Each hunt must have exactly one starting node.');
      return;
    }
    ...
    const fullHuntData: FullHuntDataForCreation = {
      ...
      is_public:
        huntMetadata.is_public !== undefined ? huntMetadata.is_public : true,
    ...
    try {
      console.log(
        'Attempting to save hunt data (client-side):',
        JSON.stringify(fullHuntData, null, 2)
      ); // Log before sending
      const result = await saveFullHunt(fullHuntData);
      setSuccessMessage(
        `Adventure "${huntMetadata.title}" crafted! ID: ${result.hunt_id}`
      );
      ...
      setError(
        err.message || 'Failed to save adventure. Check console for details.'
      );
