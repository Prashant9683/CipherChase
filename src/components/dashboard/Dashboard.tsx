...
const DashboardSection: React.FC<DashboardSectionProps> = ({
  ...
  return (
    <Card
      className={`shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out ${className}`}
    >
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          {actionLink && !isLoading && !error && (
            ...
            >
              {actionLink.icon && (
                <span className="mr-1">{actionLink.icon}</span>
              )}
              {actionLink.text}
            </Link>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {profile ? (
            <div className="space-y-2 text-sm text-gray-700">
              <p className="flex items-center">
                <FileText size={16} className="mr-2 text-gray-500" /> Hunts
                Completed:{' '}
                <span className="font-semibold ml-1">
                  {profile.total_hunts_completed || 0}
                </span>
              </p>
              <p className="flex items-center">
                <LayoutGrid size={16} className="mr-2 text-gray-500" /> Puzzles
                Solved:{' '}
                <span className="font-semibold ml-1">
        ...
        >
          {achievements && achievements.length > 0 ? (
            ...
            <p className="text-sm text-gray-500">
              No achievements unlocked yet. Complete hunts and puzzles to earn
              them!
            </p>
          )}
        </DashboardSection>
        ...
        >
          {settings ? (
            <div className="space-y-2 text-sm text-gray-700">
              <p className="flex items-center">
                <Palette size={16} className="mr-2 text-gray-500" /> Current
                Theme:{' '}
            ...
            <p className="text-sm text-gray-500">
              Customize your layout, theme, and widgets.
            </p>
          )}
        </DashboardSection>
