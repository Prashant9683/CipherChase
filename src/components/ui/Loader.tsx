...
export const Loader: React.FC<LoaderProps> = ({
    ...
    const spinnerColorClasses = variant === 'primary'
    ...
    const messageColorClasses = variant === 'primary'
        ? 'text-gray-700'
        : 'text-gray-600';
    ...
    return (
        <div className={`flex flex-col items-center justify-center space-y-3 ${className}`}>
            ...
            >
                <span className="sr-only">Loading...</span> {/* Accessibility: hidden but read by screen readers */}
            </div>
            {message && <p className={`text-sm font-medium ${messageColorClasses}`}>{message}</p>}
        </div>
    );
};
