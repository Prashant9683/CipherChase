...
const StoryNodeEditor: React.FC<StoryNodeEditorProps> = ({
  nodes,
  setNodes,
  puzzles,
  setPuzzles,
  ...
  const updateNode = (clientId: string, updates: Partial<ClientStoryNode>) => {
    setNodes((prev) =>
      prev.map((node) =>
        node.client_id === clientId ? { ...node, ...updates } : node
      )
    );
  };
  ...
  const deleteNode = (clientId: string) => {
    ...
    if (nodeToDelete && nodeToDelete.is_starting_node && nodes.length > 1) {
      alert(
        'Cannot delete the starting node if other nodes exist. Please designate another starting node first.'
      );
      return;
    }
      ...
      <CardHeader>
        ...
        {!startingNodeExists && nodes.length > 0 && (
          <p className="text-sm text-red-600 mt-2 flex items-center">
            <AlertTriangle size={16} className="mr-1.5" /> Warning: No starting
            node designated for this hunt!
          </p>
        )}
      </CardHeader>
      <CardContent>
        {nodes.length === 0 ? (
          <p className="text-slate-500 text-center py-8">
            No story nodes yet. Add one to begin crafting your narrative!
          </p>
        ) : (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <GripVertical
                        size={20}
                        className="text-slate-400 mr-2 cursor-grab"
                      />{' '}
                      {/* Placeholder for drag handle */}
                      <span className="font-semibold text-slate-700">
                        Node {node.display_order + 1}:{' '}
                        {node.node_type
                          .replace('_', ' ')
                          .replace(/\b\w/g, (l) => l.toUpperCase())}
                  ...
                  {editingNodeClientId === node.client_id && editingNode && (
                    <NodeConfigurationPanel
                      ...
                      onAddPuzzle={(newPuzzle) =>
                        setPuzzles((prev) => [
                          ...prev,
                          { ...newPuzzle, client_id: uuidv4() },
                        ])
                      } // Add new puzzle to global list
                    />
                  )}
                </li>
              ))}
