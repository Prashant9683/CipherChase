...
} from '../utils/ciphers/substitution'; // File [8]
import {
  encryptTransposition,
  decryptTransposition,
} from '../utils/ciphers/transposition'; // File [9]
  ...
  switch (cipherType) {
    case 'caesar':
      ...
      return encryptCaesar(
        plaintext,
        config?.shift !== undefined ? config.shift : 3
      );
    case 'atbash':
    ...
    case 'substitution':
      ...
      return encryptSubstitution(
        plaintext,
        (config?.key as string) || generateSubstitutionKey()
      );
    case 'transposition':
      // Default key (columns) to 5 if not provided, as in your index.ts
      return encryptTransposition(
        plaintext,
        (config?.key as number) !== undefined ? (config.key as number) : 5
      );
    case 'mirror':
      return config?.advanced
        ? mirrorTextAdvanced(plaintext)
        : mirrorText(plaintext);
    case 'morse':
...
export const decryptText = (
  ...
  switch (cipherType) {
    ...
    case 'anagram':
      ...
      return `Anagram of: "${ciphertext}". Solution might be: "${
        config?.solution || 'Not provided'
      }". Use 'Verify Anagram' for checking.`;
    case 'substitution':
    ...
    case 'mirror':
      ...
      return config?.advanced
        ? mirrorTextAdvanced(ciphertext)
        : mirrorText(ciphertext);
    case 'morse':
      return morseToText(ciphertext);
    case 'binary':
      return binaryToText(ciphertext);
    case 'riddle':
      return `Riddle: "${ciphertext}". Solution might be: "${
        config?.solution || 'Not provided'
      }".`;
    default:
...
export const validateSolution = (
  ...
  switch (cipherType) {
    ...
    case 'riddle':
      ...
      return (
        userInputSolution.trim().toLowerCase() ===
        (config?.solution || '').trim().toLowerCase()
      );
    default:
      // For other ciphers, decrypt the puzzle's ciphertext and compare with user's input solution
      try {
        const decryptedOriginal = decryptText(
          puzzleCiphertext,
          cipherType,
          config
        );
        return (
          userInputSolution.trim().toLowerCase() ===
          decryptedOriginal.trim().toLowerCase()
        );
      } catch (e) {
