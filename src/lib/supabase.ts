...
// Progress tracking
export const trackHuntCompletion = async (
  userId: string,
  huntId: string,
  timeToComplete: number
) => {
  // Record the completion
  const { error } = await supabase.from('user_hunt_progress').insert({
    user_id: userId,
    hunt_id: huntId,
...
export const trackPuzzleSolved = async (userId: string, puzzleId: string) => {
  // Record the solution
  const { error } = await supabase.from('user_puzzle_progress').insert({
    user_id: userId,
    puzzle_id: puzzleId,
